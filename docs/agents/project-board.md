...
- **Name**: arc-workflow
- **URL**: <https://github.com/users/mikeartee/projects/11>
- **Owner type**: `user`
- **Owner login**: mikeartee
- **Number**: 11
- **Node ID**: `PVT_kwHOBIHFbs4BZsim` *(used in GraphQL mutations)*

## Status field

- **Field name**: `Status`
- **Field ID**: `PVTSSF_lAHOBIHFbs4BZsimzhUpbK0`
- **Options** *(option name → option ID)*:
  - `Backlog` → `eaf6d311`
  - `Ready` → `33b62ab2`
  - `In progress` → `0e712a57`
  - `In review` → `04ab91ce`
  - `Done` → `90139f80`
...
2. Queries the issue's project items via GraphQL:
   ```bash
   gh api graphql -f query='query { repository(owner:"mikeartee", name:"arc-workflow") { issue(number:N) { projectItems(first:20) { nodes { id project { id } } } } } }'
   ```
